    ...
    function html5Upload() {	
    	...
    	sUploadURL= location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.jsp'; 	//upload URL
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			var aPhotoInfo,	htData;
			var sContents = new Array();
			...
			for(var i = 0; i <oFileInfo.length; i++){			
				...
				aPhotoInfo = {
						...
						sWidth : htData.width,
						sHeight : htData.height
					};
				sContents[i] = makePhotoTag(aPhotoInfo);
			}
			opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', [sContents]);
		}
	}
 	
 	function makePhotoTag(htPhotoInfo){
		// id와 class는 썸네일과 연관이 많습니다. 수정시 썸네일 영역도 Test
		var sTag = '<img src="{=sOriginalImageURL}" title="{=sName}" width="{=sWidth}" heigt="{=sHeight}">';
