...
function getFormHTML(data) {
	...
	$.get('/admin/mapping//urljson.do', {menu_id : data.menu_id}, function (res) {
...
function insertMenu() {
	...
	$.post('/admin/mapping/mergejson.do', param, function (data) {
		alert('맵핑이 완료되었습니다.');
	}).fail(function() {
		alert('시스템 오류가 발생했습니다.');
	});
...
$(function () {
	...
	$.get('/admin/menu/menujson.do', {}, function (data) {
	...
	sideArea.css({position : 'absolute', top : '150px', right : function () {
		return ($(this).outerWidth() * -1) - 2;
	}}).show();
	
	$(window).scroll(function () {
		var top = $(document).scrollTop() + 150;
		sideArea.css({top : top});
	}).scroll();
