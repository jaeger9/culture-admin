...
<c:if test="${not empty dataList}">
			<c:set var="suvey_total" value='0' />
			<c:set var="now_total" value='0' />
			<c:set var="rowCount" value='0' />			
			<c:forEach var="list" items="${dataList}" varStatus="sts">
				<tr>
					<td>
						<a href='/meta/quality/form.do?group_id=<c:out value='${list.JOB_GROUP_ID }' />&qs=${paramMap.qs }'>
							<c:out value='${list.JOB_GROUP_NAME }' />
						</a>
					</td>
					<td class="tmp1">
						<fmt:formatNumber pattern="#,###" value="${list.SURVEY_COUNT }" />
					</td>
					<td class="tmp2">
						<fmt:formatNumber pattern="#,###" value="${list.NOW_COUNT }" />
					</td>
					<td>
						<%--
							<c:if test="${list.SURVEY_COUNT > 0 }">
								<fmt:formatNumber pattern="#,###.##" value="${(list.NOW_COUNT / list.SURVEY_COUNT) * 100 }" /> %
							</c:if>
							<c:if test="${list.SURVEY_COUNT < 1 }">
								<c:if test="${list.NOW_COUNT > 0 }">
									∞ %
								</c:if>
								<c:if test="${list.NOW_COUNT < 1 }">
									0 %
								</c:if>
							</c:if>
							--%>
					</td>
					<c:set var="suvey_total" value='${suvey_total + list.SURVEY_COUNT }' />
					<c:set var="now_total" value='${now_total + list.NOW_COUNT }' />
					<c:set var="rowCount" value='${rowCount + 1 }' />
				</tr>
</c:forEach>
				<tr style="background-color:#F0F0F0">
					<td><c:out value='${rowCount }' />개 기관 / 합계</td>
					<td class="tmp1"><fmt:formatNumber pattern="#,###" value='${suvey_total }' /></td>
					<td class="tmp2"><fmt:formatNumber pattern="#,###" value='${now_total }' /></td>
					<td>
						<%--
							<c:if test="${suvey_total > 0 }">
								<fmt:formatNumber pattern="#,###.##" value='${(now_total / suvey_total) * 100}' /> %
							</c:if>
							<c:if test="${suvey_total < 1 }">
								<c:if test="${now_total > 0 }">
									∞ %
								</c:if>
								<c:if test="${now_total < 1 }">
									0 %
								</c:if>
							</c:if>
						--%>
