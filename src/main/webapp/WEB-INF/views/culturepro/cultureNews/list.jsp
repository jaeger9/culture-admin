...
$(function () {
	...
	// 앱 게시 여부 radio check
	if ('${paramMap.app_approval_yn}') {
		$('input:radio[name="app_approval_yn"][value="${paramMap.app_approval_yn}"]').prop('checked', 'checked');
	} else {
		$('input:radio[name="app_approval_yn"][value=""]').prop('checked', 'checked');
	}
	...
	view = function(seq) { 
		url = '/culturepro/cultureNews/view.do';
		if(seq)
			url += '?seq=' + seq;
	...
	$('span > button').each(function() {
    	$(this).click(function() { 
        	if($(this).html() == '삭제') {
        		...
        		deleteSites();
        	} else if($(this).html() == '게시') {
        		if (!confirm('게시 처리하시겠습니까?')) {
        			return false;
        		}
				$('input[name=updateStatus]').val('Y');
				updateStatus();
        	} else if($(this).html() == '미게시') {
        		if (!confirm('미게시 처리하시겠습니까?')) {
        			return false;
        		}
	...
	updateStatus = function () {
		...
		if(getCheckBoxCheckCnt() == 0) {
			if($('input[name=updateStatus]').val() == 'N')
				alert('미게시할 소식을 선택하세요');
			if($('input[name=updateStatus]').val() == 'Y')
				alert('게시할 소식을 선택하세요');
			return false;
		}
		
		formSubmit('/culturepro/cultureNews/statusUpdate.do');
	}
	...
	deleteSites = function () {
		...
		formSubmit('/culturepro/cultureNews/delete.do');
	}
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/culturepro/cultureNews/list.do">
		...
		<fieldset class="searchBox">
			...
			<div class="tableWrite">
				<table summary="문화소식관리 글 검색">
					<caption>문화소식관리</caption>
					<colgroup>
						<col style="width:13%" />
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<div class="inputBox">
									<label><input type="radio" name="app_approval_yn" value=""/> 전체</label>
									<label><input type="radio" name="app_approval_yn" value="W"/> 대기</label>
									<label><input type="radio" name="app_approval_yn" value="Y"/> 게시</label>
									<label><input type="radio" name="app_approval_yn" value="N"/> 미게시</label>
								</div>
							</td>
						</tr>
						<tr>
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${num}</td>
							<td class="subject">
								<a href="/culturepro/cultureNews/view.do?seq=${item.seq}">
									<c:out value="${item.title}"/>
								</a>
							</td>
							<td><c:out value="${item.source}"/></td>
							<td>${item.reg_date }</td>
							<td>${item.app_view_cnt}</td>
							<td>${item.app_approval_nm}</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
	...
	<div class="btnBox">
		<span class="btn white"><button type="button">게시</button></span>
		<span class="btn white"><button type="button">미게시</button></span>
