...
$(function () {
	...
	show = function(menu_cd , seq) { 
		location.href('/common/reply/list.do?rUrl=/festival/event/list.do&menu_cd=' + menu_cd + '&amp;seq=' + seq);
	...
	view = function(uci) { 
		url = '/festival/event/view.do';
		if(uci)
			url += '?uci=' + uci.replace(/\+/g,"%2B");
	...
	updateStatus = function () {
		...
		formSubmit('/festival/event/statusUpdate.do');
	}
	...
	deleteSites = function () { 
		...
		formSubmit('/festival/event/delete.do');
	}
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/festival/event/list.do">
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${item.genre}</td>
							<td>
								<a href="/festival/event/view.do?uci=<c:out value="${item.uci}"/>">
							...
							<td>${item.approval}</td>
							<td>
								<input type="hidden" name="menuTD"/><a href="/common/reply/list.do?menu_cd=1&amp;seq=<c:out value="${item.seq }" />&amp;uci=<c:out value="${item.uci }" />" class="btnN"><span class="btn whiteS"><button type="button" onClick="javascript:show('1' , '${item.seq }')">보기</button></span></a>(<c:out value="${item.reply_cnt}" />)
							</td>
						</tr>
					</c:forEach>
				</tbody>
