...
$(function () {
	...
	var start_date = frm.find('input[name=period_start]');
	var end_date = frm.find('input[name=period_end]');
	...
	if('${paramMap.apply_type}')$("select[name=apply_type]").val('${paramMap.apply_type}').attr("selected", "selected");
	if('${paramMap.periodGubun}')$("select[name=periodGubun]").val('${paramMap.periodGubun}').attr("selected", "selected");
	//검색
	...
	view = function(seq) { 
		url = '/festival/education/class/view.do';
		if(seq)
	...
	updateStatus = function () {
		...
		formSubmit('/festival/education/class/statusUpdate.do');
	}
	...
	deleteSites = function () { 
		...
		formSubmit('/festival/education/class/delete.do');
	}
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/festival/education/class/list.do">
			...
			<div class="tableWrite">
				<table summary="게시판 글 검색">
					...
					<tbody>
						...
						<tr>
							<th scope="row">교육/모집 기간</th>
							<td colspan="3">
								<select title="교육/모집 기간을  선택하세요" name="periodGubun">
									<option value="">교육/모집 기간을 선택하세요.</option>
									<option value="education">교육기간</option>
									<option value="apply">모집기간</option>
									<option value="insert">등록일</option>
								</select>
								<input type="text" name="period_start" value="${paramMap.period_start}" />
								<span>~</span>
								<input type="text" name="period_end" value="${paramMap.period_end}" />
							</td>
						</tr>
						<tr>
							...
							<td>
								<select title=">모집구분 선택하세요" name="apply_type">
									<option value="">전체</option>
									<option value="ad">홍보</option>
									<option value="apply">모집</option>
				...
				<tbody>
					...
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${num}</td>
							<td>
								<a href="/festival/education/class/view.do?seq=<c:out value="${item.seq}"/>">
							...
							<td>${item.view_cnt}</td>
							<td>
								<c:if test="${item.apply_type eq 'ad' }">
									홍보
								</c:if>
								<c:if test="${item.apply_type ne 'ad' }">
									<span class="btn whiteS"><a href="/festival/education/apply/list.do?seq=${item.seq}">신청현황(${item.apply_cnt})</a></span>
								</c:if>
							</td>
							<td>${item.approval}</td>
						</tr>
