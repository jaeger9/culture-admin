...
$(function () {
	...
	//checkbox
	new Checkbox('input[name=applySeqAll]', 'input[name=applySeq]');
	...
	updateStatus = function () {
		...
		formSubmit('/festival/education/apply/statusUpdate.do');
	}
	...
	deleteSites = function () { 
		...
		formSubmit('/festival/education/apply/delete.do');
	}
	
	//체크 박스 count 수 
	getCheckBoxCheckCnt = function() { 
		return $('input[name=applySeq]:checked').length;
	};
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/festival/education/apply/list.do">
				...
				<table summary="게시판 글 검색">
					...
					<tbody>
						...
						<tr>
							...
							<td colspan="3">
								<select title="승인여부를 선택하세요" name="approval">
		...
		<div class="tableList">
			<table summary="게시판 글 목록">
				...
				<thead>
					<tr>
						<th scope="col"><input type="checkbox" name="applySeqAll" title="리스트 전체 선택" /></th>
						<th scope="col">번호</th>
						<th scope="col">신청자</th>
						<th scope="col">연락처</th>
						<th scope="col">이메일</th>
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							<td><input type="checkbox" name="applySeq" value="${item.seq}"/></td>
							<%-- <td>${item.code}</td> --%>
							<td>${num}</td>
							<td>${item.user_id}</td>
							<td>${item.tel}</td>
