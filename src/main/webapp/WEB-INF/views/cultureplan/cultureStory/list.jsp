...
$(function () {
	...
	var ajaxApproval = function (approval) {
		...
		$.ajax({
			url			:	'/cultureplan/cultureStory/approval.do'
			,type		:	'post'
...
<form name="frm" method="get" action="/cultureplan/cultureStory/list.do">
					...
					<td>
						<select name="code">
									...
									<c:forEach var="item" items="${contList}">
										<option value="${item.code}" ${item.code eq paramMap.code ? 'selected=selected' : ''}>${item.name}</option>
									</c:forEach>
						</select>
					</td>
				</tr>
	...
	<table summary="게시판 글 목록">
		...
		<tbody>
			...
			<tr>
				...
				<td>${item.code}</td>	
				<td class="subject">
					<a href="/cultureplan/cultureStory/view.do?seq=${item.seq }&qs=${paramMap.qs }">
						${item.title }
					</a>
				</td>
				<td>${item.reg_date}</td>
...
<div class="btnBox">
	...
	<span class="btn dark fr"><a href="/cultureplan/cultureStory/view.do?qs=${paramMap.qs }">등록</a></span>
</div>
</form>
