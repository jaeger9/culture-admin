...
<c:forEach items="${categoryList }" var="j">
							<option value="${j.value }" <c:if test="${paramMap.subject eq j.value }">selected</c:if>>${j.name }</option>
</c:forEach>
						</select>
					</td>
				</tr>
...
<div class="tableList">
	<table summary="게시판 글 목록">
		...
		<thead>
			<tr>
				<th scope="col"><input type="checkbox" name="idxsAll" /></th>
				<th scope="col">번호</th>
				<th scope="col">구분</th>
				<th scope="col">기관명</th>
				<th scope="col">B</th>
		...
		<tbody>
			...
			<tr>
				<td>
					<input type="checkbox" name="idxs" value="${item.idx }" />
				</td>
				<td>
				...
				<td class="subject">
					<a href="/customer/sns/form.do?idx=${item.idx }&qs=${paramMap.qs }">
						${item.creator }
					</a>
				</td>
				<td>
...
<div class="btnBox">
	...
	<span class="btn dark fr"><a href="/customer/sns/form.do?qs=${paramMap.qs }">등록</a></span>
</div>
</form>
