				...
				<tr>
					...
					<td>
						<select name="category">
							...
							<option value="0" <c:if test="${paramMap.category eq '0'}">selected</c:if>>회원가입</option>
							<option value="1" <c:if test="${paramMap.category eq '1'}">selected</c:if>>로그인</option>
							<option value="2" <c:if test="${paramMap.category eq '2'}">selected</c:if>>문화포털 서비스</option>
							<option value="3" <c:if test="${paramMap.category eq '3'}">selected</c:if>>통합검색</option>
							<option value="4" <c:if test="${paramMap.category eq '4'}">selected</c:if>>문화PD</option>
							<option value="5" <c:if test="${paramMap.category eq '5'}">selected</c:if>>문화7거리</option>
							<option value="6" <c:if test="${paramMap.category eq '6'}">selected</c:if>>UPC</option>
							<option value="7" <c:if test="${paramMap.category eq '7'}">selected</c:if>>전통문양신청</option>
							<option value="8" <c:if test="${paramMap.category eq '8'}">selected</c:if>>오픈 API</option>
						</select>
					</td>
				</tr>
				<tr>
	...
	<table summary="게시판 글 목록">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<c:if test="${item.category eq '0'}">회원가입</c:if>
					<c:if test="${item.category eq '1'}">로그인</c:if>
					<c:if test="${item.category eq '2'}">문화포털 서비스</c:if>
					<c:if test="${item.category eq '3'}">통합검색</c:if>
					<c:if test="${item.category eq '4'}">문화PD</c:if>
					<c:if test="${item.category eq '5'}">문화7거리</c:if>
					<c:if test="${item.category eq '6'}">UPC</c:if>
					<c:if test="${item.category eq '7'}">전통문양신청</c:if>
					<c:if test="${item.category eq '8'}">오픈 API</c:if>
				</td>
				<td class="subject">
					<a href="/customer/faq/form.do?seq=${item.seq }&qs=${paramMap.qs }">
						${item.title }
					</a>
				</td>
				<td>
...
<div class="btnBox">
	...
	<span class="btn dark fr"><a href="/customer/faq/form.do?qs=${paramMap.qs }">등록</a></span>
</div>
</form>
