...
//		oEditors.getById['contents'].exec("UPDATE_CONTENTS_FIELD", []);
	...
	if ($('.delete_btn').size() > 0) {
		$('.delete_btn').click(function () {
			...
			if (datasource.val() == 'CUL') {
				param.cul_ucis = [ uci.val() ];
			} else {
				param.rdf_ucis = [ uci.val() ];
			}

			$.ajax({
				url			:	'/event/news/delete.do'
				,type		:	'post'
...
<form name="frm" method="POST" action="/event/news/form.do" enctype="multipart/form-data">
<fieldset class="searchBox">
	<legend>상세 보기</legend>

	<input type="hidden" name="qs" value="${paramMap.qr }" />
	<input type="hidden" name="uci" value="${view.uci }" />
	<input type="hidden" name="type" value="${empty view.type ? 47 : view.type }" />
	<input type="hidden" name="datasource" value="${empty view.datasource ? 'CUL' : view.datasource }" />
...
<c:if test="${not empty view.uci }">
				...
				<tr>
					...
					<td colspan="3">
						...
						<div class="upload_pop_msg">(외부 이미지 URL 또는 125px * 95px 크기의 이미지를 선택해 주세요.)</div>
						<div class="upload_pop_img">
							<c:if test="${not empty view.reference_identifier }">
								<img src="${view.reference_identifier }" width="125" height="95" alt="" />
							</c:if>
						</div>
					</td>
				</tr>
				...
				<tr>
					...
					<td colspan="3">
						<label><input type="radio" name="approval" value="Y" ${fn:trim(view.approval) ne 'N' ? 'checked="checked"' : '' } /> 승인</label>
						<label><input type="radio" name="approval" value="N" ${empty fn:trim(view.approval) or fn:trim(view.approval) eq 'N' ? 'checked="checked"' : '' } /> 미승인</label>
					</td>
				</tr>
			</tbody>
		</table>
	...
	<div class="btnBox textRight">
		...
		<span class="btn gray"><a href="/event/news/list.do?${paramMap.qr_dec }" class="list_btn">목록</a></span>
	</div>
