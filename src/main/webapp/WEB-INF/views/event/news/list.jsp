...
<form name="frm" method="get" action="/event/news/list.do">
	...
	<table summary="게시판 글 목록">
		...
		<tbody>
			...
			<tr>
				...
				<td class="subject">
					<a href="/event/news/form.do?uci=${item.uci }&qs=${paramMap.qs }">
						${item.title }
					</a>
				</td>
				<td>
				...
				<td>
					<c:if test="${fn:trim(item.approval) eq 'N'}">미승인</c:if>
					<c:if test="${fn:trim(item.approval) eq 'Y'}">승인</c:if>
				</td>
			</tr>
			</c:forEach>
...
<div class="btnBox">
	...
	<span class="btn dark fr"><a href="/event/news/form.do?qs=${paramMap.qs }">등록</a></span>
</div>
</form>
