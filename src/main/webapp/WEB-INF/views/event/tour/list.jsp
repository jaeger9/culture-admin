...
$(function () {
	...
	//checkbox
	new Checkbox('input[name=pcn_bnoAll]', 'input[name=pcn_bno]');
	
	//selectbox
	if('${paramMap.xparea}')$("select[name=xparea]").val('${paramMap.xparea}').attr("selected", "selected");
	...
	view = function(pcn_bno) { 
		url = '/event/tour/view.do';
		if(pcn_bno)
			url += '?pcn_bno=' + pcn_bno;
	...
	deleteSites = function () { 
		...
		formSubmit('/event/tour/delete.do');
	}
	
	//체크 박스 count 수 
	getCheckBoxCheckCnt = function() { 
		return $('input[name=pcn_bno]:checked').length;
	};
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/event/tour/list.do">
		...
		<fieldset class="searchBox">
			...
			<div class="tableWrite">
				<table summary="지역관광정보 검색">
					<caption>지역관광정보 검색</caption>
					...
					<tbody>
						...
						<tr>
							<th scope="row">검색어</th>
							<td colspan="3">
								<input type="text" name="sWord" title="검색어 입력" value="${paramMap.sWord}"/>
		...
		<div class="tableList">
			<table summary="게시판 글 목록">
				...
				<thead>
					<tr>
						<th scope="col"><input type="checkbox" name="pcn_bnoAll" title="리스트 전체 선택" /></th>
						<th scope="col">번호</th>
						<th scope="col">지역</th>
						<th scope="col">타입</th>
						<th scope="col">제목</th>
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							<td><input type="checkbox" name="pcn_bno" value="${item.pcn_bno}"/></td>
							<td>${num}</td>
							<td>${item.loc}</td>
							<td>${item.ctypename}</td>
							<td>
								<a href="/event/tour/view.do?pcn_bno=<c:out value="${item.pcn_bno}"/>">
