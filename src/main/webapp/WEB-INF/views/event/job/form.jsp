...
<form name="frm" method="POST" action="/event/job/form.do" enctype="multipart/form-data">
<fieldset class="searchBox">
	<legend>상세 보기</legend>

	<input type="hidden" name="qs" value="${paramMap.qr }" />
	<input type="hidden" name="uci" value="${view.uci }" />
	<input type="hidden" name="type" value="${empty view.type ? 52 : view.type }" />
	<input type="hidden" name="datasource" value="${empty view.datasource ? 'CUL' : view.datasource }" />
	<input type="hidden" name="description" value="${view.description }" />
	<input type="hidden" name="reference_identifier" value="${view.reference_identifier }" />
...
<c:if test="${not empty view.uci }">
	...
	<div class="btnBox textRight">
		...
		<span class="btn gray"><a href="/event/job/list.do?${paramMap.qr_dec }" class="list_btn">목록</a></span>
	</div>
