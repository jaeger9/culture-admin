...
$(function () {
	...
	//checkbox
	new Checkbox('input[name=idxAll]', 'input[name=idx]');
	...
	updateStatus = function () {
		...
		formSubmit('/main/keyword/statusUpdate.do');
	}
	...
	deleteKeyword = function () { 
		...
		formSubmit('/main/keyword/delete.do');
	}
	
	//체크 박스 count 수 
	getCheckBoxCheckCnt = function() { 
		return $('input[name=idx]:checked').length;
	};
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="post" action="/main/keyword/list.do">
				...
				<table summary="게시판 글 검색">
					...
					<tbody>
						<tr>
							<th scope="row">구분</th>
							<td>
								<select type="text" name="searchMobileYn" title="노출구분">
									<option value="" ${empty paramMap.searchMobileYn ? 'selected="selected"':''}>전체</option>
									<option value="N" ${paramMap.searchMobileYn eq 'N' ? 'selected="selected"':''}>포털</option>
									<option value="Y" ${paramMap.searchMobileYn eq 'Y' ? 'selected="selected"':''}>모바일</option>
								</select>
							</td>
							<th scope="row">검색어</th>
							<td>
		...
		<div class="tableList">
			<table summary="게시판 글 목록">
				...
				<thead>
					<tr>
						<th scope="col"><input type="checkbox" name="idxAll" title="리스트 전체 선택" /></th>
						<th scope="col">번호</th>
						<th scope="col">노출구분</th>
						<th scope="col">추천검색어</th>
						<th scope="col">등록일</th>
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						<tr>
							<td><input type="checkbox" name="idx" value="${item.idx}"/></td>
							<td>${item.num}</td>
							<td>${item.mobile_yn}</td>
							<td><a href="/main/keyword/view.do?idx=<c:out value="${item.idx}"/>" /><c:out value="${item.keyword}" /></a></td>
							<td>${item.reg_date}</td>
							<td>${item.approval}</td>
						</tr>
					</c:forEach>
