...
$(function () {
	...
	*/
	...
	updateStatus = function () {
		...
		formSubmit('/facility/place/rentalStatusUpdate.do');
	}
	...
	$(".btnLayer").click(function (){
		...
		$(this).next(".tooltipBox").css("display","block");
	});
	$(".tooltipBox > button").click(function (){
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/facility/place/rentalapplylist.do">
		<input type="hidden" name="page_no" value="${paramMap.page_no }"/>
		<input type="hidden" name="updateStatus" value=""/> 
		<input type="hidden" name="cul_seq" value="${paramMap.cul_seq }"/>
		<fieldset class="searchBox">
			<legend>검색</legend>
			<div class="tableWrite">
				<table summary="시설예약 접수현황 검색">
					<caption>시설예약 접수현황 검색</caption>
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${item.tel}</td>
							<td>${item.email}</td>
							<td>${item.rental_dt }</td>
							<td>${item.start_time }시~${item.end_time }시</td>
							<td>${item.person}</td>
							<td>
								<button type="button" class="btn whiteS btnLayer">보기</button>
								<!-- 사용용도 레이어팝업 : S -->
								<div class="tooltipBox" style="display:none;border:1px solid #bbb;padding:10px;width:250px;background:#fff;z-index:100;position:absolute;">
									<p>${item.purpose }
									</p>
