...
$(function () {
	...
	view = function(seq) { 
		url = '/facility/bookStore/view.do';
		if(seq)
			url += '?seq=' + seq;
	...
	updateStatus = function () {
		...
		formSubmit('/facility/bookStore/statusUpdate.do');
	}
	...
	deleteSites = function () { 
		...
		formSubmit('/facility/bookStore/delete.do');
	}
	...
	// 파일업로드
	$('span#csvRegist').click(function () {
		//Popup.cvsFileLoad('bookStore','csv');		//	기존 데이터에 추가로 저장
		//Popup.cvsFileLoad('bookStore','csv_deleteUpdate');		//	기존 데이터 삭제후 저장
		Popup.cvsFileLoad('bookStore','excel');		//	기존 데이터 삭제후 저장
		return false;
	});
});
</script>
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/facility/bookStore/list.do">
		...
		<fieldset class="searchBox">
			...
			<div class="tableWrite">
				<table summary="체육시설정보 검색">
					<caption>체육시설정보 검색</caption>
					<colgroup>
						<col style="width: 15%" />
						<col style="width: 85%" />
					</colgroup>
					<tbody>
						...
						<tr>
							<th scope="row">시설명</th>
							<td colspan="3">
								<input type="text" name="title" title="검색어 입력" value="${paramMap.title}" style="width:350px;"/>
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${num}</td>
							<td><a href="/facility/bookStore/view.do?seq=<c:out value="${item.seq}"/>" /><c:out value="${item.title}" /></a></td>
							<td>${item.address}</td>
							<td>
								<c:if test="${item.approval eq 'W' || item.approval eq null }">대기</c:if>
								<c:if test="${item.approval eq 'Y' }">승인</c:if>
								<c:if test="${item.approval eq 'N' }">미승인</c:if>
							</td>
						</tr>
					</c:forEach>
				</tbody>
	...
	<div class="btnBox">
		...
		<span class="btn dark fr" id="view" style="margin: 0px 0px 0px 5px;"><button type="button">등록</button></span>
		<!-- <span class="btn dark fr" id="csvRegist"><button type="button">서점 엑셀 파일 등록</button></span> -->
