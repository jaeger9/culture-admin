...
$(function () {
	...
	view = function(seq) { 
		url = '/facility/athletics/view.do';
		if(seq)
			url += '?seq=' + seq;
	...
	updateStatus = function () {
		...
		formSubmit('/facility/athletics/statusUpdate.do');
	}
	...
	deleteSites = function () { 
		...
		formSubmit('/facility/athletics/delete.do');
	}
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/facility/athletics/list.do">
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${num}</td>
							<td>${item.cate1 }</td>
							<td><a href="/facility/athletics/view.do?seq=<c:out value="${item.seq}"/>" /><c:out value="${item.title}" /></a></td>
							<td>${item.address}</td>
							<td>
								<c:if test="${item.approval eq 'W' }">대기</c:if>
