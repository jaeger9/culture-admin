					...
					<tbody>
						...
						<tr>
							...
							<td colspan="3">
								<select title="단체구분" name="group_type_code" id="group_type_code">
									<option value="" ${paramMap.group_type_code eq '' ? 'selected="selected"':''}>단체구분</option>							
									<c:forEach  items="${facilityGroupTypeList}" var="li">
										<option value="${li.code}" ${paramMap.group_type_code eq li.code ? 'selected="selected"':''}>${li.name}</option>							
									</c:forEach>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td colspan="3">
								...
								<input type="text" name="keyword" title="검색어 입력" value="${paramMap.name}"/>
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${num}</td>
							<td><a href="/facility/group/view.do?seq=<c:out value="${item.seq}"/>" /><c:out value="${item.name}" /></a></td>
							<td>${item.tel}</td>
							<td>${item.user_id}</td>
