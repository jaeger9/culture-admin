...
$(function () {
	...
	var page_seq			=	frm.find('select[name=page_seq]');
	...
	var ajaxApproval = function (approval) {
		...
		$.ajax({
			url			:	'/resource/board/approval.do'
			,type		:	'post'
	...
	page_seq.change(function () {
		menu_seq.html('<option value="">전체</option>');

		if ($(this).val() != '') {
			$.get('/resource/board/menujson.do', {page_seq : $(this).val()}, function (data) {
				var list = data.menuList;
				
				if (list != null && list.length > 0) {
					for (var i in list) {
						menu_seq.append('<option value="' + list[i].seq + '"' + (list[i].seq == '${paramMap.menu_seq}' ? 'selected="selected"' : '') + '  >  ' + list[i].title + '</option>');
					}
				}
			});
		}
...
<form name="frm" method="get" action="/resource/board/list.do">
		...
		<table summary="게시판 글 검색">
			...
			<tbody>
				<tr>
					<th scope="row">구분</th>
					<td>
						<select name="page_seq" style="width:150px;">
							<option value="">전체</option>
							<c:forEach items="${siteList }" var="item">
								<option value="${item.seq }" ${item.seq eq paramMap.page_seq ? ' selected="selected" ' : '' } >${item.title }</option>
							</c:forEach>
						</select>
				...
				<tr>
					...
					<td>
						<select name="search_type">
							...
							<option value="page_seq" 	${paramMap.search_type eq 'page_seq' 	? 'selected="selected"' : '' }>페이지명</option>
							<option value="menu_seq" 	${paramMap.search_type eq 'menu_seq' 	? 'selected="selected"' : '' }>메뉴명</option>
							<option value="title" 		${paramMap.search_type eq 'title' 		? 'selected="selected"' : '' }>제목</option>
							<option value="contents" 	${paramMap.search_type eq 'contents' 	? 'selected="selected"' : '' }>설명</option>
							<option value="reg_id" 		${paramMap.search_type eq 'reg_id' 		? 'selected="selected"' : '' }>등록자</option>
						</select>
		...
		<tbody>
			...
			<tr>
				...
				<td style="padding:0 0 0 4px; text-align:left;">
					${item.page_title } ${not empty item.menu_title ? '&gt;':''} ${item.menu_title }
				</td>
				<td>
				...
				<td class="subject">
					<a href="/resource/board/form.do?seq=${item.seq }&qs=${paramMap.qs }">
						${item.title }
					</a>
				</td>
				<td>
...
<div class="btnBox">
	...
	<span class="btn dark fr"><a href="/resource/board/form.do?qs=${paramMap.qs }">등록</a></span>
</div>
</form>
