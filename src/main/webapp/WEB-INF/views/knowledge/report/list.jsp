...
$(function () {
	...
	new Checkbox('input[name=ucisAll]', 'input[name=rdf_ucis], input[name=cul_ucis]');
	...
	var ajaxApproval = function (approval) {
		...
		var rdf_ucis = $('input[name=rdf_ucis]:checked');
		var cul_ucis = $('input[name=cul_ucis]:checked');

		if (!confirm(approvalText + ' 처리 하시겠습니까?')) {
			return false;
		}
		if (rdf_ucis.size() == 0 && cul_ucis.size() == 0) {
			alert('선택된 항목이 없습니다.');
			return false;
		}
		...
		if (rdf_ucis.size() > 0) {
			...
			$('input[name=rdf_ucis]:checked').each(function () {
				param.rdf_ucis.push( $(this).val() );
			});
		}
		if (cul_ucis.size() > 0) {
			param.cul_ucis = [];
			
			$('input[name=cul_ucis]:checked').each(function () {
				param.cul_ucis.push( $(this).val() );
			});
		}
		
		$.ajax({
			url			:	'/knowledge/report/approval.do'
			,type		:	'post'
	...
	$('.delete_btn').click(function () {
		...
		$.ajax({
			url			:	'/knowledge/report/delete.do'
			,type		:	'post'
			,data		:	$.param(param, true)
			,dataType	:	'json'
			,success	:	function (res) {
...
<form name="frm" method="get" action="/knowledge/report/list.do">
					...
					<th scope="row">승인여부</th>
								<td>
									<select title="승인여부 선택" name="approval">
										<option value="" <c:if test="${empty paramMap.approval }">selected</c:if>>전체</option>
										<option value="N" <c:if test="${paramMap.approval eq 'N'}">selected</c:if>>미승인</option>
										<option value="Y" <c:if test="${paramMap.approval eq 'Y' }">selected</c:if>>승인</option>
										<option value="W" <c:if test="${paramMap.approval eq 'W'}">selected</c:if>>대기</option>
									</select>
								</td>
				</tr>				
				<tr>
...
<div class="tableList">
	<table summary="게시판 글 목록">
		...
		<tbody>
			...
			<tr>
				<td>
					<input type="checkbox" name="${item.datasource eq 'RDF' ? 'rdf' : 'cul' }_ucis" value="${item.uci }" />
				</td>
				<td>
					공지
				</td>
				<td class="subject">
					<a href="/knowledge/report/form.do?uci=${item.uci }&qs=${paramMap.qs }">
						${item.title }
					</a>
				</td>
				<td>
...
<div class="btnBox">
	...
	<span class="btn dark fr"><a href="/knowledge/report/form.do?qs=${paramMap.qs }">등록</a></span>
</div>
</form>
