...
//      oEditors.getById['contents'].exec("UPDATE_CONTENTS_FIELD", []);
	...
	$('.insert_btn').click(function () {
		var jobGroupId = $('input[name=job_group_id]:checked').val();
		var jobId;
		
		if (jobGroupId == 'GRP28') {
			jobId = 'JOB1212';
		} else if (jobGroupId == 'GRP51') {
			jobId = 'JOB293';
		} else if (jobGroupId == 'GRP59') {
			if($('input[id=GRP59_1]').is(':checked')){
				jobId = 'JOB1203';
			}else{
				jobId = 'JOB1204';
			}
				...
				<tr>
					...
					<tr>
						...
						<td class="onPage" colspan="3">
							<input type="radio" name="job_group_id" id="GRP59_1" value="GRP59" <c:if test="${form.job_name eq '이달의읽을만한책' or empty form  }">checked="checked"</c:if>/>이달의읽을만한책
							<input type="radio" name="job_group_id" id="GRP59_2" value="GRP59" <c:if test="${form.job_name eq '청소년권장도서' }">checked="checked"</c:if>/>청소년권장도서
							<input type="radio" name="job_group_id" id="GRP51" value="GRP51" <c:if test="${form.job_name eq '어린이추천도서' }">checked="checked"</c:if>/>어린이추천도서
							<input type="radio" name="job_group_id" id="GRP28" value="GRP28" <c:if test="${form.job_name eq '사서추천도서'}">checked</c:if>/>사서추천도서							
							<%-- <input type="radio" name="job_group_id" value="서점마스터추천도서" <c:if test="${form.job_group_id eq '발간도서' }">checked="checked"</c:if>/>서점마스터추천도서 --%>
						</td>
					</tr>
					<tr>
						<th scope="row">책제목</th>
					...
					<tr>
						...
						<td colspan="3">
							...
							<textarea id="contents" name="description" style="width:100%;height:400px;display:none;"><c:out value="${form.description }" escapeXml="true" /></textarea>
						
						</td>
					</tr>
					<tr>
						<th scope="row">출판년도/일시</th>
						<td colspan="3">
