...
$(function () {
	...
	if('${paramMap.regType}')$("select[name=regType]").val('${paramMap.regType}').attr("selected", "selected");
	...
	$('button[name=searchButton]').click(function(){
		...
		formSubmit('/perform/ticket/list.do');
	});
	...
	//엑셀 다운로드
	excelDown = function() {
		//formSubmit('/perform/ticket/excelDownload.do');
		$('form[name=frm]').attr('action', 'excelDownload.do').submit();
	...
	updateStatus = function () {
		...
		formSubmit('/perform/ticket/statusUpdate.do');
	};
	...
	deleteSites = function () { 
		...
		formSubmit('/perform/ticket/delete.do');
	};
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/perform/ticket/list.do">
		...
		<fieldset class="searchBox">
			...
			<div class="tableWrite">
				<table summary="할인티켓 검색">
					<caption>할인티켓 글 검색</caption>
					<colgroup><col style="width:15%" /><col style="width:30%" /><col style="width:15%" /><col style="width:40%" /></colgroup>
					<tbody>
						<tr>
							<th scope="row">기간검색</th>
							<td colspan="3">
								<select name="regType">
									<option value="1">할인기간</option>
									<option value="2">등록일</option>
								</select>
								<input type="text" name="start_dt" value="${paramMap.start_dt }" />
								<span>~</span>
								<input type="text" name="end_dt" value="${paramMap.end_dt }" />
							</td>
						</tr>
						<tr>
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<%-- <td>${item.code}</td> --%>
							<td>${num}</td>
							<td><a href="/perform/ticket/view.do?seq=<c:out value="${item.seq}"/>"><c:out value="${item.title}" /></a></td>
							<td><c:out value="${item.discount}"/></td>
							<td>${item.start_dt} ~ ${item.end_dt}</td>
							
							<td>${item.user_id}</td>
							<td>${item.dowload_cnt}</td>
							<td>${item.reg_date}</td>
							<td>${item.approval}</td>
							<td>${item.view_cnt}</td>
							<td>
								<c:choose>
									<c:when test="${item.status > 0 }">진행</c:when>
	...
	<div class="btnBox">
		...
		<span class="btn dark fr" id="regist"><a href="#url">등록</a></span>
		<span class="btn dark fr" style="margin-right:4px;"><a href="#url" onclick="excelDown(); return false;">엑셀다운로드(제목,할인기간,다운수)</a></span>
