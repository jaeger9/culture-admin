...
$(function () {
	...
	view = function(seq) { 
		url = '/perform/recom/' + menuType.val() + '/view.do';
		if(seq)
			url += '?seq=' + seq;
	...
	updateStatus = function () {
		...
		formSubmit('/perform/recom/' + menuType.val() + '/statusUpdate.do');
	}
	...
	deleteSites = function () { 
		...
		formSubmit('/perform/recom/' + menuType.val() + '/delete.do');
	}
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/perform/recom/${paramMap.menuType}/list.do">
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${num}</td>
							<td><a href="/perform/recom/${paramMap.menuType}/view.do?seq=<c:out value="${item.seq}"/>"><c:out value="${item.title}" /></a></td>
							<td>${item.reg_date}</td>
							<td>${item.user_id}</td>
							<td>${item.approval}</td>
						</tr>
