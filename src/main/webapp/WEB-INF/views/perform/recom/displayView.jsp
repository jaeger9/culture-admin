	  	...
	  	$(this).click(function(){
	  		if( $(this).html() == '전시선택'){
	    		performIndex = $(this).attr("index");
	      		window.open('/popup/rdfMetadataDisplay.do', 'placePopup', 'scrollbars=yes,width=400,height=300');
	    	} 
	  	});
	});
...
<body>
	...
	<div class="tableWrite">
		<%-- 전시 ${paramMap.menuType} --%>
		<form name="frm" method="post" action="/perform/recom/insert.do" enctype="multipart/form-data">
				...
				<tbody>
					...
					<c:if test="${ empty subList}">
						<c:forEach begin="0" end="4" varStatus="status">
							<tr>
								<th scope="row">전시${status.count}</th>
								<td colspan="3">
									...
									<span class="btn whiteS"><a href="#url" index="${status.count}">전시선택</a></span>
								</td>
							</tr>
						</c:forEach>
					</c:if>
							...
							<c:forEach begin="${fn:length(subList) + 1}" end="5" var="status">
								<tr>
									...
									<td colspan="3">
										...
										<span class="btn whiteS"><a href="#url" index="${status}">전시선택</a></span>
									</td>
								</tr>
							</c:forEach>
						</c:if>
