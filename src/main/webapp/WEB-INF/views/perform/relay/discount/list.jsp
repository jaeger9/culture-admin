...
$(function () {
	...
	if('${paramMap.discount}')$("select[name=discount]").val('${paramMap.discount}').attr("selected", "selected");
					...
					<tbody>
						...
						<tr>
							...
							<td colspan="3">
								...
								<span class="btn darkS"><button name="searchButton" type="button">검색</button></span>
							</td>
						</tr>
					</tbody>
				</table>
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<%-- <td>${item.code}</td> --%>
							<td>${num}</td>
							<td><a href="/perform/relay/discount/view.do?seq=<c:out value="${item.seq}"/>"><c:out value="${item.title}" /></a></td>
							...
							<td>
								<c:choose>
									<c:when test="${item.discount != 100}">${item.discount}</c:when>
									<c:when test="${item.discount == 100}">무료</c:when>
								</c:choose>
							</td>
