					...
					<tbody>
						<tr>
							...
							<td>
								...
								<select title="카테고리 선택" name="search_category">
									<option value="">전체</option>
									<option value="1">사진이벤트</option>
									<option value="2">댓글이벤트</option>
								</select>
							</td>
						</tr>
						<tr>
			...
			<table summary="게시판 글 목록">
				...
				<c:forEach var="item" items="${list }" varStatus="status">
					...
					<tr>
						<td>${num }</td>
						<td>${item.event_nm }</td>
						<td><a href="view.do?event_seq=${item.event_seq }&user_email=${item.user_email }">${item.user_nm }</a></td>
						<td><a href="view.do?event_seq=${item.event_seq }&user_email=${item.user_email }">${item.user_email }</a></td>
