...
$(function () {
	...
	view = function(seq) { 
		url = '/pattern/apply/design/view.do';
		if(seq)
	...
	updateStatus = function () {
		...
		formSubmit('/pattern/apply/design/statusUpdate.do');
	}
	...
	deleteSites = function () { 
		...
		formSubmit('/pattern/apply/design/delete.do');
	}
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/pattern/apply/design/list.do">
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${num}</td>
							<td>${item.category}</td>
							<td>${item.upct_title}</td>							
							<td><a href="/pattern/apply/design/view.do?seq=<c:out value="${item.seq}"/>"><c:out value="${item.title}" /></a></td>
							<td>${item.reg_date}</td>
							<td>${item.view_cnt}</td>
							<td>${item.status}</td>
						</tr>
