...
$(function () {
	...
	view = function(seq) { 
		url = '/pattern/apply/gallery/view.do';
		if(seq)
	...
	updateStatus = function () {
		...
		formSubmit('/pattern/apply/gallery/statusUpdate.do');
	}
	...
	deleteSites = function () { 
		...
		formSubmit('/pattern/apply/gallery/delete.do');
	}
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/pattern/apply/gallery/list.do">
		...
		<fieldset class="searchBox">
			...
			<div class="tableWrite">
				<table summary="활용갤러리  검색">
					<caption>활용갤러리 검색</caption>
						...
						<tr>
							...
							<td colspan="3">
								<select name="searchGubun">
									...
									<option value="patternName">활용문양</option>
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${num}</td>
							<td>${item.title}</td>
							<td><a href="/pattern/apply/gallery/view.do?seq=<c:out value="${item.seq}"/>" /><c:out value="${item.pattern_name}" /></a></td>
							<td>${item.reg_date}</td>
							<td>${item.view_cnt}</td>
							<td>${item.status}</td>
						</tr>
