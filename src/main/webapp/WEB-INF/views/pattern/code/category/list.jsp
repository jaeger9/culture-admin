...
$(function () {
	...
	//checkbox
	new Checkbox('input[name=cdem_codeAll]', 'input[name=cdem_code]');
	...
	view = function(cdem_code) { 
		url = '/pattern/code/category/view.do';
		if(cdem_code)
			url += '?cdem_code=' + cdem_code;
	...
	deleteSites = function () { 
		...
		formSubmit('/pattern/code/category/delete.do');
	}
	
	//체크 박스 count 수 
	getCheckBoxCheckCnt = function() { 
		return $('input[name=cdem_code]:checked').length;
	};
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/pattern/code/category/list.do">
						...
						<tr>
							...
							<td colspan="3">
								<select name="searchGubun">
				                    <option value="0">코드분류</option>
				                    <option value="1">분류명칭</option>
                				</select>
		...
		<div class="tableList">
			<table summary="게시판 글 목록">
				...
				<thead>
					<tr>
						<th scope="col"><input type="checkbox" name="cdem_codeAll" title="리스트 전체 선택" /></th>
						<th scope="col">코드분류</th>
						<th scope="col">분류명칭</th>
				...
				<tbody>
					...
					<c:if test="${not empty list }">
						<c:forEach items="${list }" var="item" varStatus="status">
							<tr>
								<td><input type="checkbox" name="cdem_code" value="${item.cdem_code}"/></td>
								<td>${item.cdem_code}</td>
								<td><a href="/pattern/code/category/view.do?cdem_code=<c:out value="${item.cdem_code}"/>" /><c:out value="${item.cdem_name}" /></a></td>
								<td>${item.cdem_desc}</td>
								<td>${item.cdem_usyn}</td>
								<td>${item.cdem_rgdt}</td>
							</tr>
						</c:forEach>
					</c:if>
				</tbody>
