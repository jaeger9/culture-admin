...
<body>
	...
	<form name="frm" method="get" action="entryList.do">
		...
		<ul class="tab">
			<li>
				<a href="/addservice/ticketStamp/entryList.do?event=ticketStamp1_1" <c:if test="${ paramMap.event eq 'ticketStamp1_1' }"> class="focus"</c:if>>
					공연기대평 이벤트 응모현황
				</a>
			</li>
			<li>
				<a href="/addservice/ticketStamp/entryList.do?event=ticketStamp1_2" <c:if test="${ paramMap.event eq 'ticketStamp1_2' }"> class="focus"</c:if>>
					공유URL 이벤트 응모현황
				</a>
			</li>
			<li>
				<a href="/addservice/ticketStamp/entryList.do?event=ticketStamp2" <c:if test="${ paramMap.event eq 'ticketStamp2' }"> class="focus"</c:if>>
					응원댓글 이벤트 응모현황
				</a>
			</li>
		</ul>
		...
		<div class="tableList">
			<table summary="게시판 글 목록">
				...
				<colgroup>
					...
					<col style="width:8%" />
					<c:choose>
						<c:when test="${ paramMap.event ne 'ticketStamp2' }">
							<col style="width:30%" />
							<col style="width:18%" />
						</c:when>
						<c:otherwise>
				...
				<thead>
					<tr>
						...
						<th scope="col">성명</th>
						<th scope="col">댓글</th>
						<c:if test="${ paramMap.event ne 'ticketStamp2' }">
							<th scope="col">이메일</th>
						</c:if>
				...
				<c:forEach var="item" items="${list }" varStatus="status">
					...
					<tr>
						...
						<c:if test="${ paramMap.event ne 'ticketStamp2' }">
						<td style="word-break:break-word; word-wrap:break-word;">${item.email}</td>
						</c:if>
