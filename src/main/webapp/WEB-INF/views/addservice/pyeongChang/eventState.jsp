...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="post" action="/addservice/pyeongChang/eventState.do">
		<input type="hidden" name="gubun" value="${paramMap.gubun}"/>
		
		<ul class="tab">
			<li>
				<a href="/addservice/pyeongChang/eventState.do?event=pyeongChang1" <c:if test="${ paramMap.event eq 'pyeongChang1' }"> class="focus"</c:if>>
					응원하기 이벤트 참여현황
				</a>
			</li>
			<li>
				<a href="/addservice/pyeongChang/eventState.do?event=pyeongChang2" <c:if test="${ paramMap.event eq 'pyeongChang2' }"> class="focus"</c:if>>
					URL공유하기 이벤트 참여현황
				</a>
			</li>
		</ul>
