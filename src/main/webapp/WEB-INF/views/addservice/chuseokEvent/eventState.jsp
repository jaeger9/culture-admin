...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="post" action="/addservice/chuseokEvent/eventState.do">
		<input type="hidden" name="gubun" value="${paramMap.gubun}"/>
		
		<ul class="tab">
			<li>
				<a href="/addservice/chuseokEvent/eventState.do?event=chuseokEvent" <c:if test="${ paramMap.event eq 'chuseokEvent' }"> class="focus"</c:if>>
					EVENT1 참여현황
				</a>
			</li>
			<li>
				<a href="/addservice/chuseokEvent/eventState.do?event=chuseokEventSNS" <c:if test="${ paramMap.event eq 'chuseokEventSNS' }"> class="focus"</c:if>>
					EVENT2 참여현황
				</a>
			</li>
		</ul>
