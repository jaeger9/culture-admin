...
<script type="text/javascript">
	...
	function excelDown() {
		$('form[name=frm]').attr('action', 'goldenWeekExcelDown.do').submit();
	}
...
<body>
	...
	<form name="frm" method="get" action="entryList.do">
		...
		<ul class="tab">
			<li>
				<a href="/addservice/goldenWeek/entryList.do?gubun=goldenWeek_Show" <c:if test="${ paramMap.gubun eq 'goldenWeek_Show' }"> class="focus"</c:if>>
					문화 영상감상 이벤트 응모현황
				</a>
			</li>
			<li>
				<a href="/addservice/goldenWeek/entryList.do?gubun=goldenWeek_Share" <c:if test="${ paramMap.gubun eq 'goldenWeek_Share' }"> class="focus"</c:if>>
					문화 공유 이벤트 응모현황
				</a>
			</li>
		</ul>
