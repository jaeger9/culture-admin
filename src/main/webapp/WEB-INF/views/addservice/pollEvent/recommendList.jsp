...
<script type="text/javascript">
	...
	$(function() {
		...
		$('button[name=searchButton]').click(function(){
			$('form[name=frm]').attr('action', 'recommendList.do');
	...
	function excelDown() {
		$('form[name=frm]').attr('action', 'recommendExcelDown.do').submit();
	}
	
	function winnerPopup() {
		window.open('recommendWinnerPopup.do', 'winnerPopup', 'scrollbars=yes,width=600,height=630');
	}
</script>
</head>
<body>
	...
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="recommendList.do">
						...
						<tr>
							...
							<td>
								<select name="entry_month">
									<option value="" <c:if test="${empty paramMap.entry_month}">selected</c:if>>전체</option>
								<c:forEach var="list" begin="1" end="12" varStatus="status">
									<option value="${ status.count }" <c:if test="${paramMap.entry_month eq status.count}">selected</c:if>>${ status.count }</option>
								</c:forEach>
								</select>
							</td>
						</tr>
