...
<script type="text/javascript">
	$(function () {
		...
		$('div.btnBox > span > button').each(function() {
			$(this).click(function() {
				$('form[name=frm]').attr('action', '/addservice/2016Olympic/commentWinnerPopup.do')
				if ($(this).html() == '추첨하기') {
		...
		deleteWinner = function () {
			...
			formSubmit('commentWinnerDelete.do');
		}
	...
	function excelDown() {
		$('form[name=frm]').attr('action', 'commentWinnerExcelDown.do').submit();
	}
...
<form name="frm" method="post" action="/addservice/2016Olympic/commentWinnerPopup.do" style="padding:20px;">
<fieldset>
	...
	<table summary="첨자">
		...
		<c:forEach var="item" items="${winnerList }">
			<tr>
				...
				<td style="word-break: break-all;"><c:out value="${item.url}"/></td>
