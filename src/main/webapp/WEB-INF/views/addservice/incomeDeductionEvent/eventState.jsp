...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="post" action="/addservice/incomeDeductionEvent/eventState.do">
		<input type="hidden" name="gubun" value="${paramMap.gubun}"/>
		
		<ul class="tab">
			<li>
				<a href="/addservice/incomeDeductionEvent/eventState.do?event=incomeDeductionEvent" <c:if test="${ paramMap.event eq 'incomeDeductionEvent' }"> class="focus"</c:if>>
					EVENT1 참여현황
				</a>
			</li>
			<li>
				<a href="/addservice/incomeDeductionEvent/eventState.do?event=incomeDeductionEventSNS" <c:if test="${ paramMap.event eq 'incomeDeductionEventSNS' }"> class="focus"</c:if>>
					EVENT2 참여현황
				</a>
			</li>
		</ul>
