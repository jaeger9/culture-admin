...
<script type="text/javascript">
	...
	$(function() {
		...
		$('button[name=searchButton]').click(function(){
			$('form[name=frm]').attr('action', 'offerList.do');
...
<body>
	...
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="offerList.do">
						...
						<tr>
							...
							<td>
								<select name="search_field">
									...
									<option value="title" <c:if test="${paramMap.search_field eq 'title' }">selected</c:if>>제목</option>
								</select>
			...
			<table summary="게시판 글 목록">
				...
				<c:forEach var="item" items="${list }" varStatus="status">
					...
					<tr>
						...
						<td><a href="/event/offer/offerView.do?seq=${item.seq}">${item.title }</a></td>
						<td>
							<c:choose>
								<c:when test="${item.category eq 'A' }">문화예술</c:when>
								<c:when test="${item.category eq 'B' }">체육</c:when>
								<c:when test="${item.category eq 'C' }">관광</c:when>
								<c:when test="${item.category eq 'D' }">문화격차 해소</c:when>
								<c:when test="${item.category eq 'E' }">한류확산</c:when>
							</c:choose>
						</td>
