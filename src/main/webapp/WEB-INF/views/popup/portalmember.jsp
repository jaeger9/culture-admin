...
$(function () {
	...
	$('.tableList a').click(function () {
		...
		if (window.opener && window.opener.callback && window.opener.callback.portalmember) {
			window.opener.callback.portalmember( data );
		} else 
...
<form name="frm" method="get" action="/popup/portalmember.do" style="padding:20px;">
<fieldset class="searchBox">
	...
	<div class="tableWrite">
		<table summary="문화포털 유저 검색">
			<caption>문화포털 유저 검색</caption>
			<colgroup>
				<col style="width:15%" />
				<col />
			</colgroup>
	...
	<table summary="유저 목록">
		...
		<tbody>
			...
			<c:forEach items="${list }" var="item" varStatus="status">
				...
				>
					<td>
						<a href="#" name="${item.name}" location="${item.name}">
							<c:out value="${item.name}" default="-" />
						</a>
					</td>
				</tr>
			</c:forEach>
