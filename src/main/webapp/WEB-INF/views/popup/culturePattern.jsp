...
<div class="tableList">
	<ul class="tab">
		<li>
			<a href="/popup/cultureExp.do?type=pattern&gbn=1" ${paramMap.gbn == 1 ? 'class="focus"':''}>제품디자인</a>
		</li>
		<li>
			<a href="/popup/cultureExp.do?type=pattern&gbn=2" ${paramMap.gbn == 2 ? 'class="focus"':''}>활용갤러리</a>
		</li>
	</ul>
		...
		<tbody>
			...
			<c:forEach items="${list }" var="item" varStatus="status">
				...
				<tr id="trData${status.index}" style="display:none;">
					<td>
						<input type="hidden" id="uci${status.index}" name="uci"	value="${item.seq }" />
						<input type="hidden" id="code${status.index}" name="code" value="${item.code}"/>
						...
						<input type="hidden" id="summary${status.index}" name="summary" value="${item.summary}" />
					</td>
				</tr>
			</c:forEach>
		</tbody>
