...
$(function () {
	...
	$('.tableList a').click(function () {
		...
		if (window.opener && window.opener.callback && window.opener.callback.culturenews) {
			window.opener.callback.culturenews( data );
		} else 
...
<form name="frm" method="get" action="/popup/culturenews.do" style="padding:20px;">
		...
		<tbody>
			...
			<c:forEach items="${list }" var="item" varStatus="status">
				...
				>
					<td>${num}</td>
					<td>
						<c:if test="${item.type eq '46'}">뉴스</c:if>
						<c:if test="${item.type eq '52'}">채용</c:if>
					</td>
					<td>
