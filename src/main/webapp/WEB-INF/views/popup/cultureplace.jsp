...
$(function () {
	...
	$('.tableList a').click(function () {
		...
		if (window.opener && window.opener.callback && window.opener.callback.cultureplace) {
			window.opener.callback.cultureplace ( data );
		} else 
...
<form name="frm" method="get" action="/popup/cultureplace.do" style="padding:20px;">
...
<div class="tableList">
	<table summary="게시판 글 목록">
		...
		<tbody>
			...
			<tr
				data-seq="${item.cul_seq}"
				data-title="${item.title}" ,  
				data-image="${item.image}" , 
				data-url="${item.url}",
				data-place="${item.place}",
				data-summary="<c:out value='${item.summary}'/>"  
			>
				<td>
