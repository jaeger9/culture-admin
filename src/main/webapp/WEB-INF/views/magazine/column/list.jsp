...
$(function () {
	...
	updateStatus = function () {
		...
		formSubmit('/magazine/column/statusUpdate.do');
	}
	...
	deleteSites = function () { 
		...
		formSubmit('/magazine/column/delete.do');
	}
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/magazine/column/list.do">
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${num}</td>
							<td>
								<a href="/magazine/column/view.do?uci=<c:out value="${fn:replace(item.uci,'+','%2B')}"/>">
							...
							<td>${item.view_cnt}</td>
							<td>${item.approval eq 'N' ? '미승인' : '승인' }</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
