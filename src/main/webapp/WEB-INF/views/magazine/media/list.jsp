...
$(function () {
	...
	// radio check
	if ('${paramMap.video_approval_state}') {
		$('input:radio[name="video_approval_state"][value="${paramMap.video_approval_state}"]').prop('checked', 'checked');
	} else {
		$('input:radio[name="video_approval_state"][value=""]').prop('checked', 'checked');
	}
	...
	view = function(video_seq) { 
		url = '/magazine/media/view.do';
		if(video_seq)
			url += '?video_seq=' + video_seq;
	...
	updateStatus = function () {
		...
		formSubmit('/magazine/media/statusUpdate.do');
	}
	...
	deleteSites = function () {
        ...
        $("input[name=video_seq]:checked").each(function(i) {
        	$('<input>').attr('name','boardSeq').attr('type','hidden').val($(this).val()).appendTo('input[name=menuType]:last');
        });
		
		formSubmit('/magazine/media/delete.do');
	}
	
	//체크 박스 count 수 
	getCheckBoxCheckCnt = function() { 
		return $('input[name=video_seq]:checked').length;
	};
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/magazine/media/list.do">
						...
						<tr>
							...
							<td>
								<select name="searchGubun" style="width: 100px;">
									...
									<option value="video_org">기관명</option>
									<option value="video_contents">영상상세정보</option>
								</select>
			...
			<table summary="게시판 글 목록">
				...
				<tbody>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							<td><input type="checkbox" name="video_seq" value="${item.video_seq}"/></td>
							<td>${num}</td>
							<td class="subject">
								<a href="/magazine/media/view.do?video_seq=${item.video_seq}">
									<c:out value="${item.video_title}"/>
								</a>
							</td>
							<td><c:out value="${item.video_org}"/></td>
							<td>${item.insert_user_id}</td>
							<td>${item.insert_date }</td>
							<td>${item.video_approval_state_name}</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
