...
$(function () {
	...
	view = function(seq) { 
		url = '/magazine/cultureagree/view.do';
		if(seq)
			url += '?seq=' + seq;
	...
	updateStatus = function () {
		...
		formSubmit('/magazine/cultureagree/statusUpdate.do');
	}
	...
	deleteSites = function () { 
		...
		formSubmit('/magazine/cultureagree/delete.do');
	}
	...
	show = function(menu_cd , seq) { 
		location.href('/common/reply/list.do?rUrl=/magazine/cultureagree/list.do&menu_cd=' + menu_cd + '&amp;seq=' + seq);
		
		return false;
	};
	
	preshow = function(seq){
		window.open('http://www.culture.go.kr/culture/themeView.do?admin_ok=ok&seq=' + seq);
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/magazine/cultureagree/list.do">
				...
				<table summary="게시판 글 검색">
					...
					<tbody>
						...
						<tr>
							...
							<th scope="row">분류</th>
							<td>
								<select title="분류 선택" name="gbn_type">
									<option value="" ${empty param.gbn_type ? 'selected="selected"':''}>전체</option>
									<option value="1" ${param.gbn_type eq '1' ? 'selected="selected"':''}>공연전시</option>
									<option value="2" ${param.gbn_type eq '2' ? 'selected="selected"':''}>행사축제</option>
									<option value="3" ${param.gbn_type eq '3' ? 'selected="selected"':''}>영화</option>
									<option value="4" ${param.gbn_type eq '4' ? 'selected="selected"':''}>관광</option>
									<option value="5" ${param.gbn_type eq '5' ? 'selected="selected"':''}>도서</option>
									<option value="6" ${param.gbn_type eq '6' ? 'selected="selected"':''}>인물</option>
									<option value="7" ${param.gbn_type eq '7' ? 'selected="selected"':''}>문화일반</option>
									<option value="8" ${param.gbn_type eq '8' ? 'selected="selected"':''}>문화공간</option>
								</select>
							</td>
						</tr>
						<tr>
				...
				<tbody>
					<c:forEach items="${topList }" var="topList" varStatus="status">
						<tr>
							...
							<td>베스트</td>
							<td class="subject">
								<a href="/magazine/cultureagree/view.do?seq=${topList.seq}">
									<c:out value="${topList.title}"/>
								</a>
							</td>
							<td><c:out value="${topList.source_name}"/></td>
							<td>${topList.author_name}</td>
							
							<td>${item.reg_date}<br/>${not empty item.open_date ? '(':'' }${item.open_date}${not empty item.open_date ? ')':'' }</td>
							<td>${topList.view_cnt}</td>
							<td>
								<input type="hidden" name="menuTD"/><a href="/common/reply/list.do?rUrl=/magazine/cultureagree/list.do&menu_cd=8&amp;seq=<c:out value="${topList.seq }" />" class="btnN"><span class="btn whiteS"><button type="button" onClick="javascript:show('8' , '${topList.seq }')">보기</button></span></a>(<c:out value="${topList.reply_cnt}" />)
							</td>
							<td>${topList.approval}</td>
							<td><a target="_blank" href="http://www.culture.go.kr/culture/themeView.do?admin_ok=ok&seq=${topList.seq}" ><span class="btn whiteS"><button type="button" onClick="javascript:preshow('${topList.seq}')">미리보기</button></span></a></td>
						</tr>
					</c:forEach>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${num}</td>
							<td class="subject">
								<a href="/magazine/cultureagree/view.do?seq=${item.seq}">
									<c:out value="${item.title}"/>
								</a>
							</td>
							<td><c:out value="${item.source_name}"/></td>
							<td>${item.author_name}</td>
							
							<td>${item.reg_date}<br/>${not empty item.open_date ? '(':'' }${item.open_date}${not empty item.open_date ? ')':'' }</td>
							<td>${item.view_cnt}</td>
							<td>
								<input type="hidden" name="menuTD"/><a href="/common/reply/list.do?rUrl=/magazine/cultureagree/list.do&amp;menu_cd=8&amp;seq=<c:out value="${item.seq }" />" class="btnN"><span class="btn whiteS"><button type="button" onClick="javascript:show('8' , '${item.seq }')">보기</button></span></a>(<c:out value="${item.reply_cnt}" />)
							</td>
							<td>${item.approval}</td>
							<td><a target="_blank" href="http://www.culture.go.kr/culture/themeView.do?admin_ok=ok&seq=${item.seq}" ><span class="btn whiteS"><button type="button" onClick="javascript:preshow('${item.seq}')">미리보기</button></span></a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
