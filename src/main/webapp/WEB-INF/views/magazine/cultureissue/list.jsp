...
$(function () {
	...
	view = function(seq) { 
		url = '/magazine/cultureissue/view.do';
		if(seq)
			url += '?seq=' + seq;
	...
	updateStatus = function () {
		...
		formSubmit('/magazine/cultureissue/statusUpdate.do');
	}
	...
	deleteSites = function () {
		...
		formSubmit('/magazine/cultureissue/delete.do');
	};
	...
	show = function(menu_cd , seq) { 
		location.href('/common/reply/list.do?rUrl=/magazine/cultureissue/list.do&menu_cd=' + menu_cd + '&amp;seq=' + seq);
		
		return false;
	};
	
	preshow = function(seq){
		window.open('http://www.culture.go.kr/cultureissue/issueView.do?admin_ok=ok&seq=' + seq);
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/magazine/cultureissue/list.do">
				...
				<table summary="게시판 글 검색">
					...
					<tbody>
						...
						<tr>
							<th scope="row">주제</th>
							<td colspan="3">
								<select title="주제 선택" name="category_seq">
									<option value="">선택</option>
									<c:forEach var="cate" items="${categoryNonePaging }">
										<option value="${cate.seq }" <c:if test="${cate.seq eq paramMap.category_seq }">selected="selected"</c:if>>${cate.category_nm }</option>
									</c:forEach>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td>
								<select title="공연/전시일 선택하세요" name="approval_yn">
									<option value="">전체</option>
		...
		<div class="tableList">
			<table summary="게시판 글 목록">
				...
				<tbody>
					<c:forEach items="${topList }" var="topItem" varStatus="status">
						<tr>
							<td><input type="checkbox" name="seq" value="${topItem.seq}"/></td>
							<td>추천</td>
							<td class="subject">
								<a href="/magazine/cultureissue/view.do?seq=${topItem.seq}">
									<c:out value="${topItem.title}"/>
								</a>
							</td>
							<td><c:out value="${topItem.source_name}"/></td>
							<td>${topItem.author_name}</td>
							
							<td>${topItem.reg_date}</td>
							<td>${topItem.view_cnt}</td>
							<td>
								<input type="hidden" name="menuTD"/><a href="/common/reply/list.do?rUrl=/magazine/cultureissue/list.do&menu_cd=10&amp;seq=<c:out value="${topItem.seq }" />" class="btnN"><span class="btn whiteS"><button type="button" onClick="javascript:show('10' , '${topItem.seq }')">보기</button></span></a>(<c:out value="${topItem.reply_cnt}" />)
							</td>
							<td>${topItem.approval_nm}</td>
							<td><a target="_blank" href="http://www.culture.go.kr/cultureissue/issueView.do?admin_ok=ok&seq=${topItem.seq}" ><span class="btn whiteS"><button type="button" onClick="javascript:preshow('${topItem.seq}')">미리보기</button></span></a></td>
						</tr>
					</c:forEach>
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${num}</td>
							<td class="subject">
								<a href="/magazine/cultureissue/view.do?seq=${item.seq}">
							...
							<td>${item.view_cnt}</td>
							<td>
								<input type="hidden" name="menuTD"/><a href="/common/reply/list.do?rUrl=/magazine/cultureissue/list.do&amp;menu_cd=10&amp;seq=<c:out value="${item.seq }" />" class="btnN"><span class="btn whiteS"><button type="button" onClick="javascript:show('10' , '${item.seq }')">보기</button></span></a>(<c:out value="${item.reply_cnt}" />)
							</td>
							<td>${item.approval_nm}</td>
							<td><a target="_blank" href="http://www.culture.go.kr/cultureissue/issueView.do?admin_ok=ok&seq=${item.seq}" ><span class="btn whiteS"><button type="button" onClick="javascript:preshow('${item.seq}')">미리보기</button></span></a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
