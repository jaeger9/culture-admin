...
$(function () {
	...
	updateStatus = function () {
		...
		formSubmit('/magazine/blog/statusUpdate.do');
	}
	...
	deleteSites = function () { 
		...
		formSubmit('/magazine/blog/delete.do');
	}
...
<body>
	<!-- 검색 필드 -->
	<form name="frm" method="get" action="/magazine/blog/list.do">
				...
				<table summary="게시판 글 검색">
					...
					<tbody>
						<tr>
							<th scope="row">분류</th>
							<td colspan="3">
								<select title="분류선택하세요" name="subject">
				...
				<tbody>
					...
					<c:forEach items="${topList }" var="topList" varStatus="status">
						<tr>
							...
							<td>베스트</td>
							<td>${topList.type}</td>
							<td><a href="/magazine/blog/view.do?seq=<c:out value="${topList.seq}"/>" /><c:out value="${topList.title}" /></a></td>
							<td>${topList.blog_url}</td>
					...
					<c:forEach items="${list }" var="item" varStatus="status">
						...
						<tr>
							...
							<td>${item.type}</td>
							<td>
								<a href="/magazine/blog/view.do?seq=<c:out value="${item.seq}"/>">
