...
@Service
public class AdminMenuManagerService {

	private static final Logger logger = LoggerFactory.getLogger(AdminMenuManagerService.class);
	...
	public List<Object> getMenuList() throws Exception {
		...
		List<Object> menuList = service.readForList("adminMenuManager.listByTree", null);
	...
	public void mergeMenu(ParamMap paramMap) throws Exception {
		...
		if (nodeCount > 0) {
			for (int i = 0; i < nodeCount; i++) {
				...
				service.insert("adminMenuManager.merge", tmp);
			}
		}
	}
	...
	public void updateMenuSort(ParamMap paramMap) throws Exception {
		...
		if (nodeCount > 0) {
			for (int i = 0; i < nodeCount; i++) {
				...
				service.save("adminMenuManager.updateMenuSort", tmp);
			}
		}
	}
