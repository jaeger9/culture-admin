...
@Service("EducationApplyStatusUpdateService")
public class EducationApplyStatusUpdateService {
	
	//설정 파일에 밖을까 하다가 말음 설정 및 프로 파일은 있으니 알아서 하시오
	private String from = "webmaster@mail.kcisa.kr";
	private String subject = "문화 포털 교육신청 알림";
	...
	public void statusUpdate(ParamMap paramMap) throws Exception { 
		...
		ckDatabaseService.save("education.apply.statusUpdate", paramMap);
		
		//MAIL ADDR , NAME GET mailInfo
		senedMail(ckDatabaseService.readForList("education.apply.mailInfo", paramMap));
	}
	...
	private void senedMail(List<Object> userInfoList) throws Exception { 
		...
		for(Object object : userInfoList) { 
			...
			html = mailForm.replaceAll("<gubun>", data.get("gubun").toString());
