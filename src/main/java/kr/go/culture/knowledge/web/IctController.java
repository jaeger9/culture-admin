...
@RequestMapping("/knowledge/ict")
@Controller
public class IctController {

	private static final Logger logger = LoggerFactory.getLogger(IctController.class);
	...
	public String list(HttpServletRequest request, ModelMap model) throws Exception {
		...
		// model.addAttribute("creatorList", service.readForList("ict.creatorList", paramMap));
		...
		model.addAttribute("topList", service.readForList("ict.list", topParam));

		return "/knowledge/ict/list";
	}
	...
	public String form(HttpServletRequest request, ModelMap model) throws Exception {
		...
		if (paramMap.isNotBlank("uci")) {
			resultMap = (CommonModel) service.readForObject("ict.view", paramMap);

			if (resultMap == null) {
				SessionMessage.empty(request);
				return "redirect:/knowledge/ict/list.do";
			}
		}
	...
	public String merge(HttpServletRequest request, ModelMap model) throws Exception {
		...
		if (paramMap.isNotBlank("uci")) {
			...
			// update
			service.save("ict.update", paramMap);
			SessionMessage.update(request);

		} else {
			// insert
			service.insert("ict.insert", paramMap);
			SessionMessage.insert(request);
		}

		return "redirect:/knowledge/ict/form.do?uci=" + paramMap.getString("uci").replace("+", "%2b") + "&qs=" + paramMap.getQREnc();
	...
	public JSONObject approval(String[] ucis, String approval, ModelMap model) throws Exception {
		...
		service.save("ict.updateApproval", paramMap);
	...
	public JSONObject delete(String[] ucis, ModelMap model) throws Exception {
		...
		ParamMap paramMap = new ParamMap();
		paramMap.putArray("ucis", ucis);
		service.delete("ict.delete", paramMap);
