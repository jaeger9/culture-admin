...
@RequestMapping("/addservice/contestNotice")
@Controller
public class ContestNoticeController {
	...
	public String list(HttpServletRequest request, ModelMap model) throws Exception {
		...
		model.addAttribute("count", (Integer) service.readForObject("contestNotice.count", paramMap));
		model.addAttribute("list", service.readForList("contestNotice.list", paramMap));

		return "/addservice/contestNotice/list";
	}
	...
	public String form(HttpServletRequest request, ModelMap model) throws Exception {
		...
		if (paramMap.isNotBlank("seq")) {
			resultMap = (CommonModel) service.readForObject("contestNotice.view", paramMap);

			if (resultMap == null) {
				SessionMessage.empty(request);
				return "redirect:/addservice/contestNotice/list.do";
			}
		}
		...
		return "/addservice/contestNotice/form";
	}
	...
	public String merge(HttpServletRequest request, ModelMap model) throws Exception {
		...
		if (paramMap.isNotBlank("seq")) {
			...
			// update
			service.save("contestNotice.update", paramMap);
			SessionMessage.update(request);

		} else {
			// insert
			service.insert("contestNotice.insert", paramMap);
			SessionMessage.insert(request);
		}

		return "redirect:/addservice/contestNotice/form.do?seq=" + paramMap.getString("seq") + "&qs=" + paramMap.getQREnc();
	...
	public JSONObject approval(String[] seqs, String approval, ModelMap model) throws Exception {
		...
		service.save("contestNotice.updateApproval", paramMap);
	...
	public JSONObject delete(String[] seqs, ModelMap model) throws Exception {
		...
		ParamMap paramMap = new ParamMap();
		paramMap.putArray("seqs", seqs);
		service.delete("contestNotice.delete", paramMap);
