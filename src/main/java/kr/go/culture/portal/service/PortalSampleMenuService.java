 ...
 */
@Service
public class PortalSampleMenuService {

	private static final Logger logger = LoggerFactory.getLogger(PortalSampleMenuService.class);
	...
	public List<Object> getMenuList() throws Exception {
		...
		List<Object> menuList = service.readForList("portalSampleMenu.listByTree", null);
	...
	public void mergeMenu(ParamMap paramMap) throws Exception {
		...
		if (nodeCount > 0) {
			for (int i = 0; i < nodeCount; i++) {
				...
				tmp.put("menu_target", paramMap.get("nodes[" + i + "][menu_target]"));
				tmp.put("user_id", paramMap.getString("user_id", "관리자"));

				service.insert("portalSampleMenu.merge", tmp);
			}
		}
	}
	...
	public void updateMenuSort(ParamMap paramMap) throws Exception {
		...
		if (nodeCount > 0) {
			for (int i = 0; i < nodeCount; i++) {
				...
				service.save("portalSampleMenu.updateMenuSort", tmp);
			}
		}
	}
